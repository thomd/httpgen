...
describe('javascript response', function() {
  ...
  it('should have length of 188 Bytes', function() {
    expect(response.headers['x-content-size']).to.equal('188 kb');
    expect(response.headers['content-length']).to.equal('188');
  });
  ...
  it('should contain javascript code', function() {
    expect(response.text).to.equal('console.time("js execution");var result = 0;var now = new Date().getTime();while(new Date().getTime() < now + (3)){result += Math.random() * Math.random()};console.timeEnd("js execution");');
  });
});
